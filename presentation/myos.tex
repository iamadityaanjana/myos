...
\begin{frame}[fragile]{Kernel --- VGA Constants and Clear Screen}
  \begin{lstlisting}[style=clang]
#define VGA_WIDTH   80
...
void clear_screen(void) {
    // blank = colour byte in high byte, space in low byte
...
\begin{frame}[fragile]{Kernel --- Printing a Character}
  \begin{lstlisting}[style=clang]
void putchar_color(char c, unsigned char color) {
    ...
    if (cursor_row >= VGA_HEIGHT) {  // scroll if past last row
        for (int row = 1; row < VGA_HEIGHT; row++)
            for (int col = 0; col < VGA_WIDTH; col++)
                VGA_MEMORY[(row-1)*VGA_WIDTH + col] =   // copy row up
...
\begin{frame}[fragile]{Kernel --- Print Helpers and \texttt{kernel\_main}}
  \begin{lstlisting}[style=clang]
...
void print(const char *str)   { print_color(str, COLOR_WHITE_ON_BLACK); }
void println(const char *str) { print(str); putchar_color('\n', COLOR_WHITE_ON_BLACK); }
...
void kernel_main(void) {
    ...
    println("Running in 32-bit Protected Mode.");
    print_color("Ready.\n",                          COLOR_GREEN_ON_BLACK);

    // Halt safely: no IDT, so we MUST disable interrupts
    __asm__ volatile(
        "cli\n"
        ".Lhalt:\n"
        "hlt\n"
...
\begin{frame}[fragile]{Inline Assembly in C --- Explained}
  \begin{lstlisting}[style=clang]
__asm__ volatile(
    "cli\n"       // disable interrupts
    ".Lhalt:\n"   // label (local: starts with . to avoid symbol clash)
    "hlt\n"       // halt CPU (enters low-power state, waits for interrupt)
    "jmp .Lhalt\n"// if NMI wakes CPU, halt again immediately
);
  \end{lstlisting}
  \vspace{6pt}
  \concept{\texttt{\_\_asm\_\_ volatile(...)}}{
    GCC's way to embed raw assembly inside C code. \texttt{volatile} tells the compiler \textbf{never optimise this away} -- it must stay in the output, in order.
  }
  \vspace{4pt}
  \warn{A plain \texttt{for(;;)\{\}} spin-loop does NOT disable interrupts. Any interrupt with no IDT causes a triple fault and resets your machine. Always use \texttt{cli + hlt}.}
\end{frame}
...
$(BUILD)/kernel.bin: kernel_entry.o kernel.o
...
\begin{frame}{What We Built --- Summary}
  \begin{columns}[T]
    ...
    \end{itemize}
    \vspace{8pt}
    \textbf{Kernel (\texttt{kernel/kernel.c})}
    \begin{itemize}
      \item VGA text output
      \item Colour support
      \item Scrolling
      \item Clear screen
    \end{itemize}
...
\begin{frame}[plain]
  \begin{center}
    ...
    \begin{tikzpicture}
      ...
      \node[font=\ttfamily\footnotesize,color=white]   at(3.5,1.5){Kernel loaded successfully.};
      \node[font=\ttfamily\footnotesize,color=white]   at(3.5,1.0){Running in 32-bit Protected Mode.};
      \node[font=\ttfamily\footnotesize,color=green]   at(3.5,0.5){Ready.};

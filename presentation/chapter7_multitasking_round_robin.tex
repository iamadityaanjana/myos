...
\title[MyOS Chapter 7]{\textbf{MyOS Chapter 7: Multitasking (Preemptive Context Switching)}}
\subtitle{Per-task stacks + IRQ frame save/restore + round-robin fairness}
\author{MyOS Project}
\date{March 2026}
...
\begin{frame}{What was implemented}
  \begin{itemize}
    ...
    \item IRQ handler returns a selected interrupt frame pointer to assembly stub
    \item Round-robin scheduler with runtime-created named background tasks
    \item Each task owns a dedicated kernel stack and saved CPU frame
...
\begin{frame}{System flow: interrupt to context switch}
\begin{center}
\begin{tikzpicture}[
  ...
  \node[box,fill=codepurple!30] (sched) at (9.6,0) {\texttt{scheduler\_schedule}};

  \node[box,fill=codeorange!35] (rr) at (6.4,-1.6) {Save current\\task ESP frame};
  \node[box,fill=codewhite!80] (run) at (9.6,-1.6) {Return next ESP\\to \texttt{iretd}};
...
uint32_t irq_handler(registers_t* regs) {
    uint32_t next = (uint32_t)regs;
    if (regs->int_no == 32) {
        timer_irq_handler();
        next = scheduler_schedule(regs, timer_get_ticks());
    }
    if (regs->int_no == 33) {
...
\end{lstlisting}
\begin{itemize}
  \item Assembly stub consumes returned ESP and switches stack before \texttt{iretd}
  \item PIC mask keeps both timer and keyboard active under one IRQ dispatcher
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Round-robin context-switch design}
\begin{lstlisting}[style=clang]
if (current_task_index >= 0) {
    task->esp = (uint32_t)current_regs;   // save preempted frame
    task->runs++;
    task->last_tick = tick_count;
    return kernel_context_esp;            // alternate back to kernel
}

next_index = find_next_active_after(rr_last_index);
running_task = &task_table[next_index];
return task_table[next_index].esp;       // restore next task frame
\end{lstlisting}
\concept{Round-robin property}{
  Scheduler rotates through active tasks and restores each saved CPU context fairly.
}
\end{frame}
...
\begin{frame}{Background task model used}
  \begin{itemize}
    ...
    \item Each task is bootstrapped with its own stack and initial interrupt-return frame
    \item Task code runs on its own stack and is preempted by timer IRQ0
    \item \texttt{runs}, \texttt{counter}, and \texttt{last\_tick} expose per-task progress and fairness
  \end{itemize}
  \concept{Purpose of demo tasks}{
    Keep workload deterministic while proving genuine save/restore context switching.
  }
\end{frame}
...
\begin{frame}{Are these tasks real?}
  \begin{itemize}
    ...
    \item Yes: CPU register frames are saved and restored from task-owned stacks
    \item The environment is QEMU-emulated hardware, but scheduling logic is native OS code
  \end{itemize}
  \concept{Interpretation}{
    This chapter now includes full kernel-mode preemptive context switching
    with interrupt-frame stack handoff.
  }
\end{frame}
...
\begin{frame}{Keyword glossary}
  ...
  \begin{tabular}{@{}p{3.0cm}p{9.0cm}@{}}
    ...
    \texttt{Context switch} & Save current task frame and restore another task frame on IRQ return. \\
    \texttt{Background task} & Task with dedicated stack/frame preempted by timer interrupts. \\
...
\begin{frame}{Core module changes}
  ...
  \begin{tabular}{@{}p{2.7cm}p{9.3cm}@{}}
    ...
    \texttt{scheduler.c/h} & Task stacks, saved ESP frames, and round-robin context selector \\
    \texttt{isr.c} & IRQ0 now returns next execution frame from scheduler \\
    \texttt{interrupt\_stubs.asm} & IRQ common stub switches ESP to returned frame before \texttt{iretd} \\
...
\begin{frame}{Limits and next evolution}
  \begin{itemize}
    \item Context switching is currently kernel-mode only (ring 0 tasks)
    \item Tasks currently share kernel address space (no per-process VM yet)
  \end{itemize}
  \warn{This stage delivers true preemptive task frame switching and sets up process-grade multitasking next.}
\end{frame}

\begin{frame}{Outcome}
  \begin{itemize}
    \item MyOS now performs timer-driven preemptive context switching
    \item Background tasks run on dedicated stacks with saved/restored CPU frames
    \item Background tasks remain controllable from shell in real time
    \item Timer-driven OS clock and uptime are continuously maintained
    \item Kernel architecture is now ready for user-mode process evolution
  \end{itemize}
  \vspace{8pt}
  \begin{center}
    \Large \textbf{Chapter 7 now includes real preemptive CPU-context switching.}
  \end{center}
\end{frame}
